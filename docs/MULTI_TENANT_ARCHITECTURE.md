...
- 订阅成功的原子边界是：用户关系层 `subscriptions` 写入成功
- 新增订阅后的 feed 同步、文章入库和日报重算属于后续异步任务，不与订阅写入构成单事务原子提交
- 如果后续同步或日报更新失败，不回滚已经成功建立的订阅关系
- 对用户语义应保持清晰：
  - “已订阅”表示关系已生效
  - “正在同步内容 / 日报稍后更新”表示后续任务仍在进行
- 这一用户语义对所有信息源统一成立：
  - `Substack`
  - `RSS`
  - `YouTube`
- 因此，系统不以“首次同步是否完成”或“当次日报是否已更新”作为订阅成功与否的判断标准
- 系统采用这一边界，是为了保证：
  - 用户关系建立稳定
  - 全局资产复用不被瞬时抓取失败破坏
  - 日报更新允许按任务模型重试、补算和最终一致完成
...
- 手动生成日报的本质是：
  ...
  - 仅在缓存未命中时调用 AI，并把结果写回全局 `articles`
...
- 一篇文章全局只存一份
- 多个用户的 digest 可以共同引用
- 中文和英文摘要按文章维度缓存，供不同用户复用
- `summary_zh` / `summary_en` 保存结构化摘要 JSON
